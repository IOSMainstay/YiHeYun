...
- (void)viewDidLoad {
    ...
    [self configUI];
   self.title = @"血压";
    _bgScrollView.contentSize = CGSizeMake(0, MAX(_whiteView.maxY+80, _bgScrollView.height+1));
...
- (void)configUI{
    ...
    chartView = [[SCChart alloc] initwithSCChartDataFrame:CGRectMake(0, 60, [UIScreen mainScreen].bounds.size.width - 20, 180)
                                               withSource:self
                                                withStyle:SCChartLineStyle];
    [chartView showInView:_whiteView];
    chartView.lineChart.yUnit = @"压力值/mmHg";
    chartView.lineChart.xUnit = @"时间";
}
...
- (NSArray *)SCChart_yValueArray:(SCChart *)chart {
    ...
    for (NSInteger i = 0; i < 20; i++) {
        CGFloat num = arc4random_uniform(1000) / 10;
...
- (NSArray *)SCChart_ColorArray:(SCChart *)chart {
    return @[kColorFromRGBHex(0xFF3468),kColorFromRGBHex(0x4FD2C2),SCGreen];
}
