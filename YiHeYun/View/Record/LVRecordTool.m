...
- (void)startRecording {
    ...
    NSTimer *timer = [NSTimer timerWithTimeInterval:1 target:self selector:@selector(updateImage) userInfo:nil repeats:YES];
...
- (void)updateImage {
    ...
    if ([self.delegate respondsToSelector:@selector(recordTool:didstartRecoring:andTime:)]) {
        [self.delegate recordTool:self didstartRecoring: no andTime:timeCount];
    }
}
