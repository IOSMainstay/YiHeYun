...
- (void)initValues
{
    ...
    backImg.image = [[UIImage imageNamed:@"navigation_bar"] stretchableImageWithLeftCapWidth:self.bounds.size.width/4 topCapHeight:10];
...
//                                   lineBreakMode:NSLineBreakByTruncatingTail];
        //累计每个tab文字的长度
        textSize.width = self.bounds.size.width/_viewArray.count;
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin+10, 0, textSize.width-20, kHeightOfTopScrollView/*_shadowImage.size.height*/);
            button.selected = YES;
        }
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x+10, 0, sender.frame.size.width-20, kHeightOfTopScrollView/*_shadowImage.size.height*/)];
