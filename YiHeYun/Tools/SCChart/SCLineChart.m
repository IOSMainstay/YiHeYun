...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    for (int i=0; i<rowCount+1; i++) {
        if ([_ShowHorizonLine[i] integerValue]>0) {
            ...
            [path addLineToPoint:CGPointMake(UUYLabelwidth*arr.count+UUYLabelwidth,UULabelHeight+i*levelHeight)];
            [path closePath];
...
//    _xLabelWidth = (self.frame.size.width - UUYLabelwidth)/7;
    ...
    for (int i=0; i<xLabels.count; i++) {
        ...
        SCChartLabel * label = [[SCChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth+_xLabelWidth/2.0, self.frame.size.height - UULabelHeight, _xLabelWidth, UULabelHeight)];
...
-(void)strokeChart
{
    for (int i=0; i<_yValues.count; i++) {
        ...
        [bgScrollView.layer addSublayer:_chartLine];
        
        UIBezierPath *progressline = [UIBezierPath bezierPath];
        CGFloat firstValue = [[childAry objectAtIndex:0] floatValue];
        CGFloat xPosition = (UUYLabelwidth + _xLabelWidth/1.0);
        CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
